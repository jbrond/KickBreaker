...
- (void)orientationChanged:(NSNotification *)notification
{
    ...
    if (gameRunning && positionGameTime[orientation]==0 && previousOrientation != orientation) {
        ...
        //need to make sure there is no hit from the rotation
        waitTime = 20;
    }
...

{
    ...
    //Need to wait from the rotation has finished
    //until the accelerometer has stablized
    if (waitTime>0) {
        waitTime--;
    ...
    switch (cubeAction) {
        case hitAction: 
            if (gameRunning && positionGameTime[currentOrientation]==0) {
        ...
        case shakeAction: //[self unRegisterShakeSound];
            break;
        case throwAction: //[self reloadGunSound];
...
-(void) updateGameTimer
{
    ...
    if (gameRunning) {
        if (gameTime>=MAX_GAME_TIME || allDone) {
            ...
            NSString * gameEndMessage = @"Game over";
            ...
            if (allDone) {
                ...
                gameEndMessage = [NSString stringWithFormat:@"Tid Sidste Spil: %ld.%ld Sekunder",sec,msec];
            ...
            //[alert show];
            //Give the time feedback as speech
            // text to speech Your time is xx seconds and
